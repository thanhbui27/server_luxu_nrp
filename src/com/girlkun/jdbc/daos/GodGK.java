						...
						if (!rs.first()) {
							...
							try {
								...
								if (MapService.gI().isMapDoanhTrai(mapId) || MapService.gI().isMapBlackBallWar(mapId)
										...
										|| MapService.gI().isnguhs(mapId) || MapService.gI().isMapConDuongRanDoc(mapId)
										|| MapService.gI().isMapKhiGaHuyDiet(mapId)
		...
		try {
			...
			if (rs.first()) {
				...
				try {
					...
					player.location.y = Integer.parseInt(String.valueOf(dataArray.get(2)));
					if (MapService.gI().isMapDoanhTrai(mapId) || MapService.gI().isMapBlackBallWar(mapId)
							|| MapService.gI().isMapBanDoKhoBau(mapId) || MapService.gI().isMapConDuongRanDoc(mapId)
							|| MapService.gI().isMapKhiGaHuyDiet(mapId)) {
						mapId = player.gender + 21;
