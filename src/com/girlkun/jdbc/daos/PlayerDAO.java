    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            for (int i = 0; i < 20; i++) { // item tạo player 
                if (i == 0) { //thỏi vàng
                    opt.add(100); //id option cấm giao dịch
            ...
            try {
                ...
                if (player.isDie()) {
                    ...
                    if (MapService.gI().isMapDoanhTrai(mapId) || MapService.gI().isMapBlackBallWar(mapId)
                            || MapService.gI().isMapBanDoKhoBau(mapId) || MapService.gI().isMapMaBu(mapId) || MapService.gI().isnguhs(mapId) || MapService.gI().isMapConDuongRanDoc(mapId) || MapService.gI().isMapKhiGaHuyDiet(mapId) || MapService.gI().isMapConDuongRanDoc(mapId)) {
                        mapId = player.gender + 21;
                        x = 300;
                        y = 336;
                    }
                ...
                String query = " update player set pointEvent = ?, dhtime = ?, dhtime2 = ?, dhtime3 = ?, dhtime4 = ?, dhieu = ?, capcs = ?, capTT = ?, info_achievement =?, data_may_do = ?, khi_gas =?, data_item_time_sieu_cap = ?,head = ?, have_tennis_space_ship = ?,"
                        ...
                        + " skills_shortcut = ?, pointPvp=?,diemdanh=?, NguHanhSonPoint=?,data_card=?,bill_data =? ,info_phoban =? ,data_duoi_khi =? where id = ?";
