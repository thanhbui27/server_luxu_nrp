    ...
    private void useItem(Player pl, Item item, int indexBag) {
        if (item.template.strRequire <= pl.nPoint.power) {
            switch (item.template.type) {
                ...
                case 35:
                    InventoryServiceNew.gI().itemBagToBody(pl, indexBag);
                    Service.gI().sendFlagBag(pl);//thú cưỡi mớ
                    break;
                case 11: //item bag
                    InventoryServiceNew.gI().itemBagToBody(pl, indexBag);
                ...
                default:
                    switch (item.template.id) {
                        ...
                        case 2005://hop qua skh, item 2005 xd
                            ...
                            break;
                        case 2093://hop qua tl, item 2093 td
                        case 2094://hop qua tl, item 2094 nm
                        case 2095://hop qua tl, item 2095 xd
                            UseItem.gI().ItemTL(pl, item);
                            break;
                        case 2096://bo hoa
                            ItemService.gI().OpenBoHoa(pl);
                            System.out.println("mo bo hoa do :"  );
                            break;
                        case 2097://
                           ItemService.gI().OpenBoHoaVang(pl);
                            break;
                        case 736:
...
//                listClothesReward = new int[]{randClothes(param), randClothes(param)};
    ...
    private void changePetQuy(Player player, Item item) {
        if (player.pet != null) {
            int gender = player.pet.gender;
            PetService.gI().changeQuyPet(player, gender);
            InventoryServiceNew.gI().subQuantityItemsBag(player, item, 1);
        } else {
            Service.gI().sendThongBao(player, "Không thể thực hiện");
        }
    ...
    private void ItemTL(Player pl, Item item) {//hop qua do huy diet
        NpcService.gI().createMenuConMeo(pl, item.template.id, -1, "Hãy chọn một món quà", "Áo", "Quần", "Găng", "Giày", "Rada", "Từ Chối");
    }
