...
public class PetService {
        ...
        new Thread(() -> {
            try {
                ...
                Service.gI().chatJustForMe(player, player.pet, "Xin hãy thu nhận làm đệ tử");
                 ...
                 System.out.println("loi tai perServiec : " + e.getMessage());
            }
        }).start();
    }
    ...
    } public void createQuyPet(Player player, byte... limitPower) {
        new Thread(() -> {
            try {
                createNewPetQuy(player, false);
                if (limitPower != null && limitPower.length == 1) {
                    player.pet.nPoint.limitPower = limitPower[0];
                }
                Thread.sleep(1000);
                Service.gI().chatJustForMe(player, player.pet, "Sư Phụ Samurai Blue hiện thân tụi m quỳ xuống...");
            } catch (Exception e) {
                 System.out.println("loi tai perServiec : " + e.getMessage());
            }
        }).start();
    ...
    public void createQuyPet(Player player, int gender, byte... limitPower) {
        new Thread(() -> {
            try {
                createNewPetQuy(player,true, (byte) gender);
                if (limitPower != null && limitPower.length == 1) {
                    player.pet.nPoint.limitPower = limitPower[0];
                }
                Thread.sleep(1000);
    ...
    public void changePicPet(Player player, int gender) {
    ...
    } public void changeQuyPet(Player player) {
        byte limitPower = player.pet.nPoint.limitPower;
        if (player.fusion.typeFusion != ConstPlayer.NON_FUSION) {
            player.pet.unFusion();
        }
        ...
        player.pet.dispose();
        player.pet = null;
        createQuyPet(player, limitPower);
    }

    public void changeQuyPet(Player player, int gender) {
        byte limitPower = player.pet.nPoint.limitPower;
        if (player.fusion.typeFusion != ConstPlayer.NON_FUSION) {
            player.pet.unFusion();
        }
        ...
        player.pet.dispose();
        player.pet = null;
        createQuyPet(player, gender, limitPower);
    }
    ...
    private int[] getDataPetQuy() {
        int[] hpmp = {18000, 19000, 20000, 21000, 22000,20300};
        int[] petData = new int[5];
        petData[0] = Util.nextInt(40, 1150) * 20; //hp
        petData[1] = Util.nextInt(40, 1150) * 20; //mp
        petData[2] = Util.nextInt(70, 1400); //dame
        petData[3] = Util.nextInt(9, 500); //def
        petData[4] = Util.nextInt(0, 20); //crit
        return petData;
    }
    ...
    private void createNewPetQuy(Player player, boolean isQuy, byte... gender) {
        int[] data = isQuy ? getDataPetQuy(): getDataPetNormal();
        Pet pet = new Pet(player);
        pet.name = "$" + (isQuy ? "Samurai Blue" : "Đệ tử");
        pet.gender = (gender != null && gender.length != 0) ? gender[0] : (byte) Util.nextInt(0, 2);
        pet.id = -player.id;
        pet.nPoint.power = isQuy ? 15000000 : 20000;
        pet.typePet = (byte) (isQuy ? 4 : 0);
        pet.nPoint.stamina = 1000;
        pet.nPoint.maxStamina = 1000;
        pet.nPoint.hpg = data[0];
        pet.nPoint.mpg = data[1];
