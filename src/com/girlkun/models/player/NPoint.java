    ...
    private void setHpMax() {
        ...
        if (player.capCS > 0) {
            if (player.capCS <= 10) {
                ...
                dameg += (100) * player.capCS;
            }
            if (player.capCS <= 20 && player.capCS > 10) {
                hpMax +=(2000) * (player.capCS);
                dameg += (200) * player.capCS;
            }
            if (player.capCS > 20) {
                hpMax +=(3000) * (player.capCS);
                dameg += (300) * player.capCS;
            }
        }
         if (player.capTT > 0) {
            if (player.capTT <= 10) {
                hpMax += (4000) * player.capTT;
                dameg += (400) * player.capCS;
            }
            if (player.capTT <= 20 && player.capTT > 10) {
                hpMax +=(5000) * (player.capTT);
                dameg += (500) * player.capCS;
            }
            if (player.capTT > 20) {
                hpMax +=(6000) * (player.capTT);
                dameg += (600) * player.capCS;
            }
        }
...
//        }
        ...
        dameAttack = Util.nextInt((int) (dameAttack - ((long) dameAttack * 2 / 100)), (int) (dameAttack + ((long) dameAttack * 2 / 100)));
        if (player.isPl()) {
