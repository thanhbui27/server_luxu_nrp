...
//    public static byte RATE_EXP_SERVER = 1;// sau khi chinh
    ...
    public static final String queryTopTangHOA = "SELECT id, CAST( pointEvent AS UNSIGNED) AS pointEvent FROM player ORDER BY CAST( pointEvent AS UNSIGNED) DESC LIMIT 50;";
    public static final String queryTopNap = "SELECT p.id, a.coin FROM player p JOIN account a ON p.account_id = a.id ORDER BY a.coin DESC LIMIT 10;";
    ...
    public static List<TOP> tangHoa;    
            ...
            for (File fileEntry : folder.listFiles()) {
                if (!fileEntry.isDirectory()) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        ItemMobReward item  = null;
                         
                        if(itemId != 2077 || itemId != 1107){
                             item = new ItemMobReward(itemId, mapDrop,
...
//                RUBY_REWARDS.add(Util.sendDo(861, 0, new ArrayList<>()));
            ...
            tangHoa = realTop(queryTopTangHOA, con);
            Logger.success("Load top tang hoa thành công (" + topSK.size() + ")\n");
            ...
            while (rs.next()) {
                ...
                switch (query) {
                    case queryTopSM:
                        ...
                        break;
                    case queryTopNV:
                        top.setInfo1(TASKS.get(rs.getByte("nv")).name + "Điểm");
                        top.setInfo2(TASKS.get(rs.getByte("nv")).name + "Điểm");
                        break;
                    case queryTopSK:
                    ...
                    case queryTopSD:
                        ...
                        break;
                     case queryTopTangHOA:
                       top.setInfo1(rs.getInt("pointEvent") + " điểm");
                       top.setInfo2(rs.getInt("pointEvent") + " điểm");
                       break;
                     case queryTopNap:
                       top.setInfo1(rs.getInt("coin") + " VND");
                       top.setInfo2(rs.getInt("coin") + " VND");
                        break;
                }
                tops.add(top);
            }
