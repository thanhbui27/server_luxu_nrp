...
public class Mob {
   ...
   public static void initMopbMbank(Mob mob, int level) {
        if ( level <= 700){
        mob.point.dame = (level * 3250 * mob.level * 4) * 5;
        mob.point.maxHp = (level * 12472 * mob.level * 2 + level * 7263 * mob.tempId) * 5;
         }
    ...
    public long getTiemNangForPlayer(Player pl, long dame) {
        ...
        if (pl.zone.map.mapId == 173 || pl.zone.map.mapId == 179){
            tiemNang *= 4;
        }
        if (pl.capCS > 0) {
            if (pl.capCS <= 10) {
    ...
    public List<ItemMap> getItemMobReward(Player player, int x, int yEnd) {
        ...
        if (player.itemTime.isUseMayDo && Util.isTrue(21, 100) && this.tempId > 57 && this.tempId < 66) {
            list.add(new ItemMap(zone, 380, 1, x, player.location.y, player.id));
                ...
                if (player.itemTime.isUseMaydosc && Util.isTrue(7, 100) && this.tempId > 1 && this.tempId < 81) {
            list.add(new ItemMap(zone, 2036, 1, x, player.location.y, player.id));
        }
        if (player.itemTime.isUseChimUng && Util.isTrue(2, 100) && this.tempId == 182 ) {
            list.add(new ItemMap(zone, 1203, 1, x, player.location.y, player.id));
            list.add(new ItemMap(zone, 1180, 1, x, player.location.y, player.id));
        }
     
         Item item = player.inventory.itemsBody.get(1); // cải trang rớt tv
        if (this.zone.map.mapId == 159 || this.zone.map.mapId == 182){
            if(item.isNotNullItem()){
            if (item.template.id == 1194){ 
        if (Util.isTrue(5, 100)) {    
            list.add(new ItemMap(zone, Util.nextInt(457), Util.nextInt(1,5), x, player.location.y, player.id));} // Rơi thỏi vàng
