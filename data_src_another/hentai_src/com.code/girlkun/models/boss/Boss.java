...
public class Boss extends Player implements IBossNew, IBossOutfit {
   ...
   //     System.out.println("this status: " + this.bossStatus + " (" + this.id + ")");
    ...
    protected void notifyJoinMap() {
        ...
        if (this.zone.map.mapId == 140||MapService.gI().isMapMaBu(this.zone.map.mapId) || MapService.gI().isMapBlackBallWar (this.zone.map.mapId) || MapService.gI().isMapDoanhTrai(this.zone.map.mapId))
            return;
    ...
    public void die(Player plKill) {
        if (plKill != null) {
            reward(plKill);
            ServerNotify.gI().notify(plKill.name + " vừa tiêu diệt được " + this.name + ", Cả sever đều ngưỡng mộ");
        }
