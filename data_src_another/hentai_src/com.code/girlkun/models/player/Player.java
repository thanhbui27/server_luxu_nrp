...
import com.girlkun.models.map.Npcchat;
        ...
        if (!this.beforeDispose) {
            try {
                if (!iDMark.isBan()) {
                    ...
                    if (this.lastTimeTitle4 != 0 && Util.canDoWithTime(this.lastTimeTitle3, 6000)) {
                        lastTimeTitle4 = 0;
                    ...
                    if (!isBoss && this.iDMark.isGotoFuture() && Util.canDoWithTime(this.iDMark.getLastTimeGoToFuture(), 6000)) {
                     ...
                     if (this.iDMark.isGoToGas() && Util.canDoWithTime(this.iDMark.getLastTimeGotoGas(), 2000)) {
...
//                }
    ...
    public byte getAura() {
        if (this.inventory.itemsBody.isEmpty() || this.inventory.itemsBody.size() < 11 || this.inventory.itemsBody.size() < 10) {
            return -1;
        }
        ...
        if (this.inventory.itemsBody.size() == 13) {
            if (this.inventory.itemsBody.get(8).isNotNullItem()) {
               if (this.inventory.itemsBody.get(8).template.id == (1980+7)){
                ...
                if (this.inventory.itemsBody.get(8).template.id == (1980+6)){
                    return 210;
                } else {
    ...
    public float injured(Player plAtt, float damage, boolean piercing, boolean isMobAttack) {
        if (!this.isDie()) {
            ...
            if (isMobAttack && this.charms.tdBatTu > System.currentTimeMillis() && damage >= this.nPoint.hp) {
                damage = Util.NHuyHauGH(this.nPoint.hp - 1);
            }
