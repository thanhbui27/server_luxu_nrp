...
public class Input {
    ...
    public static final int kami = 514; 
    public static final int TNSMSERVER = 526;
    public static final int SEND_ITEM_OP = 512;
       public static final int CHOOSE_LEVEL_GAS = 544;

    public static final int NAP_TIEN = 508;
        ...
        try {
            ...
            switch (player.iDMark.getTypeInput()) {
                ...
                case SEND_ITEM_OP:
                    ...
                    //    int idOption = Integer.parseInt(text[2]);
                        ...
                        if (pBuffItem != null) {
                            ...
                            if (idItemBuff == -1) {
                                ...
                                Service.getInstance().sendMoney(player);
                            } else if (idItemBuff == -4) {
                                pBuffItem.inventory.vnd = Math.min(pBuffItem.inventory.vnd + slItemBuff, 2000000000);
                                txtBuff += slItemBuff + " vnd\b";
                                Service.getInstance().sendMoney(player);
                            } else {
                                //Item itemBuffTemplate = ItemBuff.getItem(idItemBuff);
                                Item itemBuffTemplate = ItemService.gI().createNewItem((short) idItemBuff);
                               // itemBuffTemplate.itemOptions.add(new Item.ItemOption(idOption, 0));
                    ...
                    if (level >= 1 && level <= 100000) {
                        ...
                        if (npc != null) {
                            npc.createOtherMenu(player, ConstNpc.MENU_ACCPET_GO_TO_GAS,
                                    "Con có muốn tới Vùng Đất thám Hiểm cấp độ " + level + "?",
                                    new String[]{"Vâng", "Từ chối"}, level);
                        }
                    } else {
               ...
               case TAI:
                    ...
                    try {
                        if (player.inventory.ruby > sotvxiu) {
                            ...
                            if (1 <= (x + y + z) && (x + y + z) <= 10) {
                                if (player != null) {
                                    player.inventory.addRuby((int) Math.round(sotvxiu * 1.8));
                                    Service.gI().sendMoney(player);
                        ...
                        if (player.inventory.ruby > sotvtai) {
                            ...
                            if (1 <= (x + y + z) && (x + y + z) <= 10) {
                                if (player != null) {// tự sửa lại tên lấy
                                    Service.gI().sendThongBaoOK(player, "Kết quả" + "\nSố hệ thống quay ra là :"
                                            + " " + x + " " + y + " " + z + "\nTổng là : " + tong + "\nBạn đã cược : "
                                            + sotvtai + " hồng ngọc vào Tài 1" + "\nKết quả : Xỉu " + "\nCòn cái nịt.");
                                    return;
                                }
                            } else if (x == y && x == z) {
                                if (player != null) {
                                    Service.gI().sendThongBaoOK(player, "Kết quả" + "Số hệ thống quay ra : " + x + " " + y + " " + z + "\nTổng là : " + tong + "\nBạn đã cược : " + sotvtai + " hồng ngọc vào Xỉu" + "\nKết quả : Tam hoa" + "\nCòn cái nịt.");
                                    return;
                                }
                            } else if ((x + y + z) > 10) {
                                if (player != null) {
                                    player.inventory.addRuby((int) Math.round(sotvtai * 1.8));
                                    Service.gI().sendMoney(player);
                ...
                case NAP_TIEN:
                    if (player.isAdmin()) {
                        ...
                        if (pBuffItem != null) {
                            int idItemBuff = Integer.parseInt(text[1]);
                            int idOptionBuff = Integer.parseInt(text[2]);
                            PlayerDAO.naptien(pBuffItem, idItemBuff, idOptionBuff);
                            Service.gI().sendThongBao(pBuffItem, "Bạn nhận được "+idOptionBuff+" vnd.");
                        } else {
    ...
    public void TAI(Player pl) {
        createForm(pl, TAI, "Chọn số hồng ngọc đặt Xỉu", new SubInput("Số hồng ngọc", ANY));//????
    }

    public void XIU(Player pl) {
        createForm(pl, XIU, "Chọn số hồng ngọc đặt Tài", new SubInput("Số hồng ngọc", ANY));
    }
    ...
    public void createFormSenditem1(Player pl) {
        createForm(pl, SEND_ITEM_OP, "SEND Vật Phẩm Option",
                ...
                new SubInput("chỉ số muốn bufff", NUMERIC),                
                new SubInput("Số lượng", NUMERIC));
    }
    public void createFormSenditem2(Player pl) {
        createForm(pl, kami, "SEND Vật Phẩm Option",
                new SubInput("Tên người chơi", ANY),
                new SubInput("ID Trang Bị", NUMERIC),
                new SubInput("ID SKH ", NUMERIC),                
                new SubInput("ID Option ", NUMERIC),
                new SubInput("chỉ số muốn bufff", NUMERIC),                
                new SubInput("Số lượng", NUMERIC));    
    }
    public void createFormNap(Player pl) {

        createForm(pl, NAP_TIEN, "Nhập đủ thông tin", new SubInput("Nhân vật: ", NUMERIC), new SubInput("Số tiền: ", NUMERIC), new SubInput("Số vnd", NUMERIC));
    }
    ...
    public void createFormChooseLevelBDKB(Player pl) {
     ...
     public void createFormChooseLevelGas(Player pl) {
        createForm(pl, CHOOSE_LEVEL_GAS, "Chọn cấp độ", new SubInput("Cấp độ (1 đến 100000)", NUMERIC));
    }
    public void createFormChangeTNSMServer(Player pl){
        createForm(pl, TNSMSERVER, "HIỆN TẠI" + Manager.RATE_EXP_SERVER, new SubInput ("TĂNG NHIÊU THÌ NHẬP ĐI  ", ANY));
    }     
