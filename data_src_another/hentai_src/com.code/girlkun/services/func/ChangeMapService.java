...
public class ChangeMapService {
            ...
            switch (pl.iDMark.getTypeChangeMap()) {
                case ConstMap.CHANGE_CAPSULE:
                    ...
                    for (int i = 0; i < pl.mapCapsule.size(); i++) {
                     ...
                     if (pl.haveDuongTang) {
            Service.getInstance().sendThongBao(pl, "Đang hộ tống Mị Nương không thể Capsun");
            return;
        }
    ...
    public void openZoneUI(Player pl) {
        if (pl.haveDuongTang) {
            Service.getInstance().sendThongBao(pl, "Đang Hộ tống Mị Nương không thể đổi khu");
            return;
        }
        if (pl.zone == null) {
    ...
    public void changeMapInYard(Player pl, int mapId, int zoneId, int x) {
        ...
        if (zoneJoin != null) {
            x = x != -1 ? x : Util.nextInt(200, zoneJoin.map.mapWidth - 200);
            changeMap(pl, zoneJoin, -1, -1, x, zoneJoin.map.yPhysicInTop(x, 200), NON_SPACE_SHIP);
        }
    }
    ...
    private void changeMap(Player pl, Zone zoneJoin, int mapId, int zoneId, int x, int y, byte typeSpace) {
        ...
        if (zoneJoin != null) {
            ...
            boolean currMapIsKhiDoc = MapService.gI().isMapKhiDoc(pl.zone.map);
            boolean nextMapIsKhiDoc = MapService.gI().isMapKhiDoc(zoneJoin.map);
            ...
            if (currMapIsCold != nextMapIsCold) {
                ...
                if (currMapIsKhiDoc != nextMapIsKhiDoc) {
                if (!currMapIsKhiDoc && nextMapIsKhiDoc) {
                    Service.gI().sendThongBao(pl, "Khí độc Bao trùm khu vực này");
                    Service.gI().sendThongBao(pl, "Sức tấn công và HP của bạn bị giảm 70% vì quá Ngộp");
                } else {
                    Service.gI().sendThongBao(pl, "Bạn đã Thoát khỏi vùng khí độc");
                    Service.gI().sendThongBao(pl, "Sức tấn công và HP của bạn đã trở lại bình thường");
                }
            ...
            Service.gI().sendThongBaoOK(pl, "Vui lòng hoàn thành nhiệm vụ");
        }
    }
