...
public class PetService {
    ...
    public void createBerusPet(Player player, int gender, byte... limitPower) {
        ...
        new Thread(() -> {
            try {
                ...
                Thread.sleep(1000);
                Service.gI().chatJustForMe(player, player.pet, "Sư Phụ SUSU hiện thân tụi m quỳ xuống...");
            } catch (Exception e) {
            }
        }).start();
    }
    ...
    private int[] getDataPetMabu() {
     ...
     private int[] getDataPetBerus() {
        int[] hpmp = {2700, 2800, 2900, 3000, 3100, 3200};
        int[] petData = new int[5];
        petData[0] = Util.nextInt(40, 125) * 20; //hp
        petData[1] = Util.nextInt(40, 125) * 20; //mp
        petData[2] = Util.nextInt(70, 150); //dame
        petData[3] = Util.nextInt(9, 50); //def
        petData[4] = Util.nextInt(0, 2); //crit
        return petData;
    }
     private int[] getDataPetPic() {
        int[] hpmp = {1800, 1900, 2000, 2100, 3200,3300};
        int[] petData = new int[5];
    ...
    private void createNewPet(Player player, boolean isMabu, boolean isBerus,boolean isPic, byte... gender) {
        ...
        pet.name = "$" + (isMabu ? "Mabư" : isBerus ? "Bắt Giới" : isPic ? "Ngộ Không" : "Đệ tử");
