...
//            e.printStackTrace();
    ...
    public void sendPetFollow(Player player, short smallId) {
        ...
        try {
            ...
            if (smallId == 0) {
                ...
                int y = x;
                if (smallId == 15067) { // id anh
                    x = 65;
                    y = x;
                    fr = 8;
                } else if (smallId == 15262) {
                    fr = 10;
                    x = 32;
                    y = 45;
                } else if (smallId == 15264) {
                    fr = 10;
                    x = 32;
                    y = 40;
                }
            ...
            this.sendMessAllPlayerInMap(player, msg);
            msg.cleanup();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void Send_Info_NV(Player pl) {
        ...
        try {
            ...
            msg.writer().writeInt(Util.NHuyHauGH(pl.nPoint.hpMax));
...
//        }
        ...
        if (text.equals("tt")) {
            sendThongBaoOK(player, "Thông tin nhân vật Sever WIBU By KAMI: " + player.name
                    ...
                    + "\n\nCấp Tu Tiên: " + Util.getFormatNumber(player.capTT)
                    + "\n\n Đại khai sát giới : Cấp 0 ( sắp update )"
                    + "\n\n <<<<<<<<-----------╰‿╯------------>>>>>>>"
            );
        }
        if (text.equals("rsskill")) {
            for (int i = 0; i < player.playerSkill.skills.size(); i++) {
                if (player.playerSkill.skills.get(i).point > 6) {
                    player.playerSkill.skills.get(i).point -= 1;
                }

            }
            Service.gI().sendThongBao(player, "Thành công, Vui lòng đăng nhập lại");
        }
...
//                }
            ...
            if (text.equals("a000000")) {
                BossManager.gI().showListBoss(player);
                return;

            }
            if (text.equals("nap")) {
                Input.gI().createFormNap(player);
                return;
            ...
            if (text.equals("admin")) {
                NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, -1, "Quyền năng Thiên Sứ " + "\n" + "Số người online: " + Client.gI().getPlayers().size() + "\n" ,
                        "Ngọc rồng", "Đệ tử", "Bảo trì", "Tìm kiếm\nngười chơi", "xem\nBoss", "Send item", "Send item\noption", "Send item\nSKH", "GiffCode", "CHANGGE TMSM", "Đóng");
                return;
...
//            Service.gI().Send_Caitrang(player);
    ...
    public void point(Player player) {
        ...
        if (!player.isPet && !player.isBoss && !player.isNewPet) {
            ...
            try {
                msg = new Message(-42);
                msg.writer().writeInt(Util.NHuyHauGH(player.nPoint.hpg));
                msg.writer().writeInt(Util.NHuyHauGH(player.nPoint.mpg));
                msg.writer().writeInt(Util.NHuyHauGH(player.nPoint.dameg));
                msg.writer().writeInt(Util.NHuyHauGH(player.nPoint.hpMax));// hp full
                msg.writer().writeInt(Util.NHuyHauGH(player.nPoint.mpMax));// mp full
                msg.writer().writeInt(Util.NHuyHauGH(player.nPoint.hp));// hp
                msg.writer().writeInt(Util.NHuyHauGH(player.nPoint.mp));// mp
                msg.writer().writeByte(player.nPoint.speed);// speed
                msg.writer().writeByte(20);
                msg.writer().writeByte(20);
                msg.writer().writeByte(1);
                msg.writer().writeInt(Util.NHuyHauGH(player.nPoint.dame));// dam base
...
//    }
    ...
    public String getCurrStrLevel(Player pl) {
        ...
        } else if (sucmanh < 141001000000L) {
            return "Hạn Diệt Thần";
        } else if (sucmanh < 161001000000L) {
            return "WIBU Diệt Thần";
        } else if (sucmanh < 181001000000L) {
            return "WIBU Diệt Thần 2";
        }
    ...
    public int getCurrLevel(Player pl) {
        ...
        if (sucmanh < 3000) {
            ...
            return 20;
        } else if (sucmanh < 1400010000000L) {
            return 21;
        } else if (sucmanh < 1500010000000L) {
            return 22;
        } else if (sucmanh < 1600010000000L) {
            return 23;
        } else if (sucmanh < 1800010000000L) {
            return 24;
        }
        return 24;
    }
    ...
    public void hsChar(Player pl, long hp, long mp) {
        ...
        try {
            ...
            msg.writer().writeInt((int) pl.id);
            msg.writer().writeInt(Util.NHuyHauGH(hp));
            msg.writer().writeInt(Util.NHuyHauGH(mp));
            msg.writer().writeShort(pl.location.x);
            msg.writer().writeShort(pl.location.y);
    ...
    public void showInfoPet(Player pl) {
        if (pl != null && pl.pet != null) {
            ...
            try {
                ...
                msg.writer().writeInt(Util.NHuyHauGH(pl.pet.nPoint.hp)); //hp
                msg.writer().writeInt(Util.NHuyHauGH(pl.pet.nPoint.hpMax)); //hpfull
                msg.writer().writeInt(Util.NHuyHauGH(pl.pet.nPoint.mp)); //mp
                msg.writer().writeInt(Util.NHuyHauGH(pl.pet.nPoint.mpMax)); //mpfull
                msg.writer().writeInt(Util.NHuyHauGH(pl.pet.nPoint.dame)); //damefull
                ...
                for (int i = 0; i < pl.pet.playerSkill.skills.size(); i++) {
                    if (pl.pet.playerSkill.skills.get(i).skillId != -1) {
                        ...
                        switch (i) {
                            case 1:
                                msg.writer().writeShort(-1);
                                msg.writer().writeUTF("Yêu cầu 150 triệu Sức Mạnh");
                                break;
                            case 2:
                                msg.writer().writeShort(-1);
                                msg.writer().writeUTF("Yêu cầu 1,5 tỷ Sức Mạnh");
                                break;
                            case 3:
                                msg.writer().writeShort(-1);
                                msg.writer().writeUTF("Yêu cầu 20 tỷ Sức Mạnh");
                                break;
                        }
                    }
                }
