...
public class ItemTimeService {
    ...
    public void sendAllItemTime(Player player) {
        ...
        if (player.itemTime.isUseCuongNo2) {
          ...
          if (player.itemTime.isUseXimuoi) {
            sendItemTime(player, 10904, (int) ((TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeXimuoi)) / 1000));
        }
        ...
        if (player.itemTime.isUseMaydosc) {
            sendItemTime(player, 16004, (int) ((TIME_MAY_DO2 - (System.currentTimeMillis() - player.itemTime.lastTimeUseMaydosc)) / 1000));
        }
        if (player.itemTime.isUseChimUng) {
            sendItemTime(player, 12148, (int) ((TIME_CHIM_UNG - (System.currentTimeMillis() - player.itemTime.lastTimeUseChimUng)) / 1000));
        }
    ...
    public void sendCanAutoPlay(Player player) {
     ...
     public void sendTextGas(Player player) {
        if (player.clan != null
            ...
            sendTextTime(player, KHI_GASS, "Vùng Đất Thám Hiểm: ", secondsLeft);
        }
    }
