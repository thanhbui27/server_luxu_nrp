...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            dataArray.add(900000000); //hồng ngọc
            ...
            dataArray.add(gender == 0 ? 2000 : 1000); //hp gốc
            dataArray.add(gender == 1 ? 2000 : 1000); //ki gốc
            dataArray.add(gender == 2 ? 105 : 100); //sức đánh gốc
            dataArray.add(0); //giáp gốc
            dataArray.add(0); //chí mạng gốc
            dataArray.add(0); //năng động
            dataArray.add(gender == 0 ? 2000 : 1000); //hp hiện tại
            dataArray.add(gender == 1 ? 2000 : 1000); //ki hiện tại
    ...
    public static void updatePlayer(Player player) {
        if (player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
               ...
               dataArray.add(player.inventory.ruby > Inventory.LIMIT_RUBY
                        ? Inventory.LIMIT_RUBY : player.inventory.ruby);
                ...
                int hp = Util.NHuyHauGH(player.nPoint.hp);
                int mp = Util.NHuyHauGH(player.nPoint.mp);
                if (player.isDie()) {
                    ...
                    if (MapService.gI().isMapDoanhTrai(mapId) || MapService.gI().isMapBlackBallWar(mapId)
                            || MapService.gI().isMapBanDoKhoBau(mapId) || MapService.gI().isMapMaBu(mapId)  || MapService.gI().isMapKhiGas(mapId)) {
                        mapId = player.gender + 21;
                        x = 300;
                        y = 336;
                    }
                ...
                dataArray.add((player.itemTime.isUseMayDo ? (ItemTime.TIME_MAY_DO2 - (System.currentTimeMillis() - player.itemTime.lastTimeUseMayDo)) : 0));
                ...
                dataArray.add((player.itemTime.isUseChimUng ? (ItemTime.TIME_CHIM_UNG - (System.currentTimeMillis() - player.itemTime.lastTimeUseChimUng)) : 0));
                dataArray.add(player.itemTime.iconMeal);
                dataArray.add((player.itemTime.isUseXimuoi ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeXimuoi)) : 0));
                ...
                dataArray.clear();
                String query = " update player set dhtime4 = ?, data_card = ?, dhtime = ?, dhtime2 = ?, dhtime3 = ?, dhieu = ?,capcs = ?,quang = ?,capTT = ?, head = ?, have_tennis_space_ship = ?,"
                        ...
                        + "data_black_ball = ?, data_side_task = ?, data_charm = ?, skills = ?, skills_shortcut = ?, pointPvp=? where id = ?";
    ...
    public static boolean naptien(Player player, int sotien, int vnd) {
        try {
            Connection con = GirlkunDB.getConnection();
            PreparedStatement ps = con.prepareStatement("update account set tongnap = (tongnap+?),vnd = (vnd + ?) where id = ?");
            ps.setInt(1, sotien);
            ps.setInt(2, vnd);
