...
import com.girlkun.utils.Util;import com.kygui.ItemKyGui;
import com.kygui.ShopKyGuiManager;import org.json.simple.JSONArray;
...
//    public static byte RATE_EXP_SERVER = 1;// sau khi chinh
    ...
    public static final String queryTopSM = "SELECT id, capCS FROM player ORDER BY capcs DESC LIMIT 20;";
    public static final String queryTopSK = "SELECT id, capTT FROM player ORDER BY capTT DESC LIMIT 20;";
    ...
    public static final String queryTopSD = "SELECT id, CAST( split_str( data_inventory,',',5)  AS UNSIGNED) AS event FROM player ORDER BY CAST( split_str( data_inventory,',',5)  AS UNSIGNED) DESC LIMIT 20;";
...
//                    dataArray.clear();
            ...
            Logger.success("Load top sd thành công (" + topSK.size() + ")\n");
    ...
    public static List<TOP> realTop(String query, Connection con) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                switch (query) {
                    case queryTopSM:
                        top.setInfo1(rs.getLong("capcs") + "");
                        top.setInfo2(rs.getLong("capcs") + "");
                        break;
                    case queryTopNV:
                        ...
                        break;
                    case queryTopSK:
                        top.setInfo1(rs.getInt("capTT") + "");
                        top.setInfo2(rs.getInt("capTT") + "");
                        break;
                    case queryTopPVP:
